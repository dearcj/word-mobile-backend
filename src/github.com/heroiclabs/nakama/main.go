...
func main() {
	...
	leaderboardCache.Create("lb_today", false, server.LeaderboardSortOrderDescending, server.LeaderboardOperatorBest, "0 0 * * *", "{}")
	leaderboardCache.Create("lb_week", false, server.LeaderboardSortOrderDescending, server.LeaderboardOperatorBest, "0 0 * * 0", "{}")
	leaderboardCache.Create("lb_month", false, server.LeaderboardSortOrderDescending, server.LeaderboardOperatorBest, "0 0 1 * *", "{}")
	...
	wx := server.CreateWordX(startupLogger, db)
	runtimePool := server.NewRuntimePool(wx, logger, startupLogger, db, config, socialClient, leaderboardCache, sessionRegistry, matchRegistry, tracker, router, stdLibs, modules, regCallbacks, once)
	...
	_, globalPath, _, _ := runtime.Caller(0)
	globalPath = filepath.Dir(globalPath)

	server.StartFileServer(logger, globalPath, wx)
	go wx.Dbcleaner.RunRoutine(logger)
	go wx.TournamentCron.RunRoutine(logger)
