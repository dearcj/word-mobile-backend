...
SELECT username, display_name, avatar_url, lang_tag, location, timezone, metadata, wallet,
	...
	create_time, update_time, verify_time, role
FROM users
WHERE id = $1`

	if err := db.QueryRow(query, userID).Scan(&username, &displayName, &avatarURL, &langTag, &locat, &timezone, &metadata, &wallet, &email, &facebook, &google, &gamecenter, &steam, &customID, &edge_count, &createTime, &updateTime, &verifyTime, &role); err != nil {
