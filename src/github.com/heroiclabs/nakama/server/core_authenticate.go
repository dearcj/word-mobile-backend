...
func UpdateUserLastLogin(db *sql.DB, logger *zap.Logger, userID string) error {
	query := "UPDATE users set (last_login) = (now()) where id = $1"
	_, err := db.Exec(query, userID)
	if err != nil {
		logger.Error("Can't update last login.", zap.Error(err))
	}
	return nil
}

func StorageValueSet(val string) string {
	x := struct {
...
func StorageValueGet(val string) (error, string) {
	x := struct {
		Value string `json:"value"`
	}{}
...
func CreateDefaultUserStorage(logger *zap.Logger, db *sql.DB, userID uuid.UUID) error {
	x := make(map[uuid.UUID][]*api.WriteStorageObject)
	x[userID] = []*api.WriteStorageObject{
		{
			Collection:      "resources",
			Key:             "money",
			Value:           StorageValueSet(DEFAULT_MONEY),
		...
		},
		{
			Collection:      "resources",
			Key:             "points",
			Value:           StorageValueSet(DEFAULT_POINTS),
			PermissionRead:  &wrappers.Int32Value{Value: 1},
			PermissionWrite: &wrappers.Int32Value{Value: 1},
		},
	}

	_, _, error := StorageWriteObjects(logger, db, true, x)
	if error != nil {
		return error
	} else {
		return nil
...
func AuthenticateCustom(logger *zap.Logger, db *sql.DB, customID, username string, create bool) (string, string, bool, error) {
	...
	userIDUUID, err := uuid.NewV4()
	userID := uuid.Must(userIDUUID, err).String()
	...
	CreateDefaultUserStorage(logger, db, userIDUUID)
...
func AuthenticateEmail(logger *zap.Logger, db *sql.DB, email, password, username string, create bool) (string, string, bool, error) {
	...
	if found {
		...
		UpdateUserLastLogin(db, logger, dbUserID)
	...
	// Create a new account.
	userIDUUID := uuid.Must(uuid.NewV4())
	...
	UpdateUserLastLogin(db, logger, userID)
	CreateDefaultUserStorage(logger, db, userIDUUID)
