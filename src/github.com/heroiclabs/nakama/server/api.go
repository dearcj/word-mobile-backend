...
func apiInterceptorFunc(logger *zap.Logger, config Config, runtimePool *RuntimePool, jsonpbMarshaler *jsonpb.Marshaler, jsonpbUnmarshaler *jsonpb.Unmarshaler) func(context.Context, interface{}, *grpc.UnaryServerInfo, grpc.UnaryHandler) (interface{}, error) {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
		...
		if beforeHookResult == req && hookErr == nil {
			runtimePool.wx.BeforeHook(logger, config, runtimePool, jsonpbMarshaler, jsonpbUnmarshaler, "", uid, username, expiry, info.FullMethod, req)
		}
		...
		if handlerErr == nil {
			...
			runtimePool.wx.AfterHook(logger, config, runtimePool, jsonpbMarshaler, "", uid, username, expiry, info.FullMethod, handlerResult)
			// Stats measurement end boundary.
			span.End()
			stats.Record(statsCtx, MetricsApiTimeSpentMsec.M(float64(time.Now().UTC().UnixNano()-startNanos)/1000), MetricsApiCount.M(1))
		}
...
func securityInterceptorFunc(logger *zap.Logger, config Config, ctx context.Context, req interface{}, info *grpc.UnaryServerInfo) (context.Context, error) {
	switch info.FullMethod {
	case "/nakama.api.Nakama/CheckCode":
		return ctx, nil
	case "/nakama.api.Nakama/GetRestoreCode":
		return ctx, nil
	case "/nakama.api.Nakama/RestorePassword":
		return ctx, nil
	case "/nakama.api.Nakama/Healthcheck":
	...
	default:
		...
		if strings.Index(methodName, "Dashboard") == 0 {
			//NEED TO CHECK ADMIN RIGHTS
			//info.Server
			serv := info.Server.(*ApiServer)
			if !serv.IsAdmin(userID) {
				return nil, status.Error(codes.PermissionDenied, "Dashboard API available only for admins")
